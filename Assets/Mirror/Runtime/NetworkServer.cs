    ...
    public static class NetworkServer
    {
        ...
        public static bool SendToReady<T>(NetworkIdentity identity, T msg, bool includeSelf = true, int channelId = Channels.DefaultReliable) where T : IMessageBase
        {
            ...
            if (identity != null && identity.observers != null)
            {
                ...
                bool localSelf = identity.connectionToClient != null && identity.connectionToClient.connectionId == 0;
                // Handle local connection
                if (identity.observers.ContainsKey(0) && (!localSelf || includeSelf))
                {
                ...
                bool result = Transport.activeTransport.ServerSendMany(identity.observers.Keys.ToArray(), channelId, 
                    bytes, includeSelf, 
                    identity.connectionToClient == null ? -1 : identity.connectionToClient.connectionId, 
                    ref count);
                Profiler.EndSample();
